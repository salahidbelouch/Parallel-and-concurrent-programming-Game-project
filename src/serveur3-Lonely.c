...
char *concat7(char const*str1, char const*str2, int count) {
    size_t const l1 = 7 ;
    size_t const l2 = count ;
...
int nouvelleCO(int port,struct sockaddr_in *servaddr,struct sockaddr_in *cliaddr,int newsockfd,socklen_t len){
	fd_set read_fds;
	FD_ZERO(&read_fds);
	int it=0;
	//char buffer[MAXLINE];
		...
		while ((notAcked>0)||(!(feof(stream)))){ //LIRE ENVOYER ATTENDRE ACK
			...
			if (windowSize>0){
				...
				if ((!(feof(stream)))){
					...
					count = fread(buffer, 1, 1474, stream);
			...
			if ((windowSize==0)||(last==1)){
					...
					struct timeval timeout;
                	timeout.tv_sec = 0;
                	timeout.tv_usec = 50;//1500 c'est mieux
					//usleep(100);
...
int handleCO(int sockfd, struct sockaddr_in srvaddr){ 
	...
	while(1){
		...
		struct sockaddr_in fservaddr, fcliaddr;
		socklen_t flen= sizeof(struct sockaddr);
		
		// Creating socket file descriptor
		if ( (fnewsockfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0 ) {
			perror("socket creation failed");
			exit(EXIT_FAILURE);
		}
		
		memset(&fservaddr, 0, sizeof(fservaddr));
		memset(&fcliaddr, 0, sizeof(fcliaddr));
		
		// Filling server information
		fservaddr.sin_family = AF_INET; // IPv4
		fservaddr.sin_addr.s_addr = INADDR_ANY;
		fservaddr.sin_port = htons(port);
		
		// Bind the socket with the server address
	 	bind(fnewsockfd, (const struct sockaddr *)&fservaddr,sizeof(fservaddr));

		res=concat7("SYN-ACK",ports,4);
		//printf("ANOTHER ? \n");
		...
		if (comp==0){ 
			...
			if (comp ==0){
				if (fork()==0){
					nouvelleCO(port,&fservaddr,&fcliaddr,fnewsockfd,len);
				}
			}
