...
int nouvelleCO(int port){
	...
	// Bind the socket with the server address
	 bind(newsockfd, (const struct sockaddr *)&servaddr,sizeof(servaddr));
		...
		int ACkdup=0;
		int datasize=200;
		char *Data[datasize][1480];//buffer contenant les messages envoyé
		int lastseg;
		int it2=0;
		int last =0;
		int dejadup=-1;
		...
		while ((notAcked>0)||(!(feof(stream)))){ //LIRE ENVOYER ATTENDRE ACK
						...
						if (it==0){
							...
							if((atoi(Nseg)==lastAck)&&(dejadup!=lastAck)){
								...
								if (ACkdup ==dupmax){ // recevoir plusieurs ACKdup
									...
									//break;
									//usleep(100);
									int sent =sendto(newsockfd, &Data[(lastAck)%(datasize)][0], 1480,
										0, (struct sockaddr *) &cliaddr,
										len);
			...
			if ((windowSize==0)||(last==1)){
					...
					struct timeval timeout;
                	timeout.tv_sec = 0;
                	timeout.tv_usec = 50;//1480 c'est mieux
					//usleep(100);
